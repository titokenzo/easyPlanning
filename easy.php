...
$logged = isset($_SESSION[User::SESSION])? $_SESSION[User::SESSION] : NULL;
...
$app->get('/loginOrganization', verifyLogin(), function () use($logged,$error) {
    if($logged["user_isadmin"]==0){
        ...
        $orgs = Organization::getOrgIdList();
    }
    $page = new Page([
        "header" => false,
        "footer" => false
...
// #########################################################################################
// USER LIST
$app->group('/users', verifyLogin(), function () use ($app,$logged,$error) {
    $app->get('/', function () use($logged) {
        $objs = User::listFromOrganization($logged['org_id']);
        $page = new Page();
    ...
    // USER VIEW CREATE
    $app->get('/create', function () use($error){
        $page = new Page();
    ...
    // USER SAVE CREATE
    $app->post('/create', function () use ($app,$logged) {
        $obj = new User();
...
// #########################################################################################
// LIST
$app->group('/plans', verifyLogin(), function () use ($app,$logged,$error) {
    $app->get('/', function () use($logged) {
        $objs = Plan::listPlansFromOrg($logged["org_id"]);
        $page = new Page();
    ...
    // VIEW
    $app->get('/view', function () use($logged) {
        $obj = Plan::getActiveOrgPlan($logged["org_id"]);
        $new = false;
        if (! $obj) {
            $new = true;
        }
...
// #########################################################################################
// LIST
$app->group('/diagnostics', verifyLogin(), function () use ($app,$logged) {
    $app->get('/', verifyLogin(), function () use($logged) {
        $objs = Diagnostic::listAllFromOrg($logged["org_id"]);
...
$app->group('/respondents', verifyLogin(), function () use ($app,$logged,$error) {
    $app->get('/', function () use($logged){
        $page = new Page();
        $page->setTpl('respondents', array(
            "objs" => Respondent::getFromActiveDiagnostic($logged["org_id"]),
...
$app->group('/objectives', verifyLogin(), function () use ($app,$logged,$error) {
    $app->get('/', function () use($app,$logged) {
        $objs = Objective::listObjectivesFromActivePlan($logged["org_id"]);
        $plan = Plan::getActiveOrgPlan($logged["org_id"]);
        if (! $plan) {
            $app->response->redirect('/', 301);
        }
        $page = new Page();
    ...
    // CREATE
    $app->get('/create', function () use($logged){
        $error = isset($_SESSION['slim.flash']['error']) ? $_SESSION['slim.flash']['error'] : NULL;
        $page = new Page();
    ...
    // VIEW UPDATE
    $app->get('/:idobj', function ($idobj) use($logged,$error) {
        $obj = new Objective();
        $obj->get((int) $idobj);
        $plan = Plan::getActiveOrgPlan($logged["org_id"]);
        $page = new Page();
    ...
    $app->get('/:idobj/targets', function ($idobj) use($logged) {
    ...
    // CREATE
    $app->get('/:idobj/targets/create', function ($idobj) use($logged,$error) {
        $objective = new Objective();
        $objective->get((int) $idobj);
        $page = new Page();
    ...
    // VIEW UPDATE
    $app->get('/:idobj/targets/:idtarget', function ($idobj, $idtarget) use($logged,$error) {
        $objective = new Objective();
        $objective->get((int) $idobj);
        $obj = new Target();
        $obj->get((int) $idtarget);
...
// #########################################################################################
// LIST
$app->group('/monitoring', verifyLogin(), function () use ($app,$logged,$error) {
    $app->get('/', function () use($app,$logged) {
        $objs = Monitoring::listPlanLastResults($logged["plan_id"]);
        if (! $objs) {
            $app->response->redirect('/', 301);
        }
        $page = new Page();
    ...
    // VIEW TARGET CREATE
    $app->get('/target/create/:idtarget', function ($idtarget) use($logged,$error) {
        $page = new Page();
        $page->setTpl('monitoring-target-create', array(
            "objs" => Monitoring::listTargetResults($idtarget),
            "obj" => Monitoring::getTargetMetaData($idtarget),
            "users" => User::listFromOrganization($logged["org_id"]),
            "frequenceUnits" => Target::getFrequenceUnitsList(),
            "error" => $error
        ));
    ...
    // VIEW TARGET UPDATE
    $app->get('/target/update/:idmon', function ($idmon) use($logged,$error) {
        $obj = new Monitoring();
        $obj->get((int) $idmon);
        $target = new Target();
    ...
    $app->get('/obj/create/:idobj', function ($idobj) use($logged,$error) {
        $objective = new Objective();
        $objective->get((int) $idobj);
...
// REPORTS
// #############################################################################################
$app->group('/reports', verifyLogin(), function () use ($app,$logged,$error) {
...
$app->group('/admin', verifyLoginAdmin(),
    function () use ($app,$logged,$error) {
        ...
        // #####################################################################################
        // USER LIST
        $app->group('/users', function () use ($app,$logged,$error) {
            $app->get('/', function () {
                $objs = User::listAll();
                $page = new Page();
